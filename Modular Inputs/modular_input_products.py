...
def collect_events(helper, ew):
    ...
    for product in r_json['products']:
        state = helper.get_check_point(str(product['id']))
        if state is None:
            final_result.append(product)
            helper.save_check_point(str(product['id']), 'indexed')
        # helper.delete_check_point(str(product["id"])) # Only required when testing modular input
